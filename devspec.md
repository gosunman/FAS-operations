...
### Captain (`src/captain/`)
- **main.ts**: 통합 캡틴 진입점. Gateway API, Output Watcher, Planning Loop, Telegram Commands, Stale Task Cleanup를 한 프로세스에서 기동. 그레이스풀 셧다운 지원. `pnpm captain`으로 실행. Output Watcher 감시 대상은 `config/agents.yml`에서 동적 로딩(`src/shared/agents_config.ts`) + 기동 시 `tmux list-sessions`로 실제 존재하는 세션만 자동 필터링 (캡틴 자체 세션 `fas-captain` 제외). 나이트 플래닝 훅에서 Feedback Extractor를 호출하여 완료 태스크의 교훈을 자동 추출 + Research Store 30일 보관 자동 정리. **Gemini discovery 활성화**: `gemini_config` 전달로 나이트 플래닝에서 동적 태스크 발견 기능 가동. **Stale cleanup**: 5분 간격으로 in_progress 30분+ 태스크를 blocked 전환 + alert 알림. **Plan C 확장**: `create_ai_usage_tracker()` 인스턴스 생성 후 gateway(ChatGPT), task_executor(Gemini), output_watcher(Claude)에 주입. 모닝 브리핑에 `get_infra_report` 콜백 전달.
- **morning_briefing.ts**: 모닝 브리핑 생성기. 야간 완료 태스크 요약 + 오늘 스케줄 + 대기/차단 태스크 현황 → Telegram/Slack + Notion 상세 백업. **Plan C 확장**: `MorningBriefingDeps.get_infra_report` 콜백 및 `BriefingData.infra_report` 필드 추가. 인프라 리포트 섹션이 야간 태스크 요약 **앞에** 배치되어 하드웨어/AI 상태를 먼저 확인. Notion 상세 백업에도 Infrastructure Report 섹션 포함.
...
- **resource_monitor.ts**: 시스템 리소스 + AI 사용량 통합 모니터. (1) `create_resource_monitor()` — macOS vm_stat/sysctl/df/ioreg/netstat로 CPU/RAM/디스크/GPU/온도/네트워크 수집, 임계값 체크(CPU>90%, RAM>85%, Disk>80%). **Plan C 확장**: `parse_gpu_usage()` (Apple Silicon ioreg gpu-core-utilization), `parse_temperature()` (ioreg die-temp), `parse_network_throughput()` (netstat -ib). `take_snapshot()`과 `collect_snapshot()`이 GPU/온도/네트워크 메트릭을 포함하여 반환. (2) `create_ai_usage_tracker()` — 프로바이더별(Claude/Gemini/ChatGPT) 요청 횟수/성공률 추적, 일일 자동 리셋, 알림 콜백(70% warning, 90% critical). (3) `create_unified_monitor()` — 시스템+AI 통합 인터페이스(`collect_snapshot()`, `check_thresholds()`, `get_ai_usage_summary()`, `start()`/`stop()`). (4) `create_telegram_alert_handler()` — sustained CPU 체크(연속 3회), 쿨다운(5분), severity 분류.
- **time_classifier.ts**: 기기 상태 분류기 (Plan C). `create_time_classifier(config)` — 1분 주기로 기기 상태를 `working`/`idle`/`down`으로 분류. **캡틴**: `pgrep`으로 프로세스(claude, node) 활성 여부 + CPU 사용률(기본 10% 임계값)로 판별. **헌터**: Gateway heartbeat API 기반. 히스토리 추적(`MachineTimeEntry[]`)과 시간 합산 요약(`working_ms`/`idle_ms`/`down_ms`) 제공.
- **daily_aggregator.ts**: 일일 인프라 리포트 생성기 (Plan C). `aggregate_snapshots()` — ResourceSnapshot[] + TimeSummary → DailyMachineStats (CPU/GPU/온도/RAM/네트워크 avg/max/min). `analyze_bottlenecks()` — 5가지 병목 감지: underutilized(idle>60%), cpu_bottleneck(avg>70%), api_limit(throttle>3), overheating(temp>90°C), memory_pressure(RAM>85%). `build_daily_report()` → DailyInfraReport 조립. `format_infra_report_telegram()` — 한국어 이모지 Telegram 메시지 포맷. 헬퍼: `format_duration(ms)`, `format_bytes(bytes)`.
- **activity_integration.ts**: 서비스 간 활동 로그 통합 훅. `create_activity_hooks(logger, ai_tracker?)` — Gateway/Router/Telegram 등 서비스의 이벤트를 ActivityLogger에 기록. **Plan C 확장**: `log_ai_call(provider, success, reason)` — SQLite 활동 로그 + AI usage tracker에 동시 보고. 선택적 `AIUsageTracker` 주입으로 라이브 AI 사용량 통계 갱신.
