...
| `usage_budget.ts` | 사용량 예산 관리 — 시간대별 배분, 쿨다운, 우선순위 큐, 경고 단계 |
| `start.ts` | Gateway + Telegram + Slack + UsageMonitor + UsageBudget + GeminiFallback 통합 엔트리포인트 |
...
## 사용량 예산 시스템 (Usage Budget)

Claude Code 사용량 소진을 사전에 방지하는 예산 기반 관리 시스템입니다. `usage_monitor.ts`(반응적 실패 감지)와 보완적으로 동작합니다.

### 핵심 메커니즘

| 메커니즘 | 설명 |
|----------|------|
| **시간대별 예산** | 하루를 4개 시간대(야간/오전/오후/저녁)로 나누고 각각 예산 비율 배분. 새벽에 모두 소진하는 것을 방지 |
| **시간당 작업 상한** | 1시간에 고비용 작업(cost >= 10) 최대 N개로 제한 (기본 3개) |
| **쿨다운 시스템** | 연속 고비용 작업 N회(기본 3회) 후 강제 쿨다운 (기본 5분) |
| **우선순위 큐** | 예산 초과 시 낮은 우선순위 작업을 큐에 보관, 여유 생기면 처리 |
...
| `none` | < 50% | 정상 운영 |
| `info` | >= 50% | Slack 정보 알림 |
| `warning` | >= 70% | Telegram 경고 알림 |
| `critical` | >= 85% | 저비용(low/normal) 작업 차단, high/emergency만 허용 |
| `emergency` | >= 95% | emergency만 허용, Gemini 폴백 준비 |

### 시간대별 예산 배분 (기본값)

| 시간대 | 시간 (UTC) | 예산 비율 |
|--------|-----------|-----------|
| night | 00:00–06:00 | 15% |
| morning | 06:00–12:00 | 30% |
| afternoon | 12:00–18:00 | 35% |
| evening | 18:00–24:00 | 20% |

### 환경 변수

| 변수 | 필수 | 설명 |
|------|------|------|
| `USAGE_DAILY_BUDGET` | X | 일일 총 예산 단위 (기본: 100) |
| `USAGE_HOURLY_MAX_HIGH_COST` | X | 시간당 고비용 작업 최대 수 (기본: 3) |
| `USAGE_COOLDOWN_CONSECUTIVE` | X | 쿨다운 발동 연속 고비용 작업 수 (기본: 3) |
| `USAGE_COOLDOWN_DURATION_MS` | X | 쿨다운 지속 시간 ms (기본: 300000 = 5분) |
...
npx vitest run tests/daemon/usage_budget.test.ts
```
