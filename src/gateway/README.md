...
| `task_store.ts` | SQLite 기반 태스크 저장소 (WAL 모드). CRUD, 상태 전환, 통계, 중복 검사, stale in_progress 감지. `action` 필드 지원 |
| `sanitizer.ts` | 개인정보 산이타이징 레이어 — PII 탐지 + severity 기반 2단계 처리 (critical: quarantine, warning: auto-sanitize) |
...
산이타이저는 2단계 severity로 PII를 분류한다:

| Severity | 동작 | 대상 패턴 |
|----------|------|----------|
| `critical` | **quarantine** — 결과 격리, 인간 검토 대기 | `resident_id`, `phone_number`, `labeled_korean_name`, `credit_card` |
| `warning` | **auto-sanitize** — PII 마스킹 후 정상 처리 (로그 경고) | `email`, `address`, `bank_account`, `financial_amount`, `ip_address`, `internal_url` |

- `bank_account` 패턴: 마지막 그룹 4자리 이상 요구 (날짜 `YYYY-MM-DD` 오탐 방지)
- `address` 패턴: 동/로/길/읍/면/리 후속 요소 필수 (일반 지역명 오탐 방지)
