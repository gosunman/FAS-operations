...
> Doctrine은 FAS 클러스터의 정신, 원칙, 정체성, 보안 설계를 담당하는 Source of Truth.
> 생성일: 2026-03-18
...
- `interface`보다 `type`을 선호한다 (union, intersection이 필요한 경우).
- `any` 사용을 피하고 `unknown`을 사용한 뒤 타입 가드로 좁힌다.
...
- `dataclass` 또는 `pydantic`을 적극 활용한다.
- async 코드 작성 시 `asyncio`를 기본으로 사용한다.
...
- `README.md`와 `devspec.md`의 업데이트는 **Stop hook**(`~/.claude/hooks/check-docs-update.sh`)에 의해 강제된다. 코드 변경 시 문서가 업데이트되지 않으면 hook이 차단한다.

### `README.md` — 모든 사람을 위한 안내서
- **대상 독자**: 프로젝트에 처음 접하는 사람 포함, 모든 사람.
- **폴더 단위로 각각 생성**한다. 루트 `README.md`가 비대해지지 않도록, 각 폴더(모듈/디렉토리)마다 해당 폴더 전용 `README.md`를 둔다.
- 루트 `README.md`는 프로젝트 전체 개요와 각 하위 폴더로의 링크/요약만 포함한다.
- 각 폴더의 `README.md`에는 다음을 포함한다:
  - 해당 폴더의 **목적**과 **역할**
  - 주요 **기능** 설명
...
### 문서 관리 참고
- 새 폴더/모듈을 생성하면 해당 폴더에 `README.md`를 함께 생성한다.
...
주인님의 **뇌** — Mac Studio #2, macOS 유저명 `user`.
...
| 에이전트 | 기기 | 역할 |
|---------|------|------|
| 섀도우(shadow) ✍️ | MacBook Pro ([MASKED_OWNER]) | 주인님의 손 — 곁에서 직접 실행 |
| 캡틴(captain) 🧠 | Mac Studio #2 (user) | 주인님의 뇌 — 판단, 전략, 오케스트레이션 |
...
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_workstyle.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_tone.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_permissions.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_planning_first.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_hunter_isolation.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_dev_languages.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_unattended_dev.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_writing_quality.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_cross_verification.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_memory_scope.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_remote_workflow.md
@~/.claude/projects/-Users-[MASKED_OWNER]/memory/feedback_auto_save.md
...
주인님의 **손** — MacBook Pro, macOS 유저명 `[MASKED_OWNER]`.
...

{
  "permissions": {
    "allow": [
      ...
      "Bash(cp:*)",
      "Bash(ssh captain:*)",
      "Bash(ICLOUD=\"/Users/[MASKED_OWNER]/Library/Mobile Documents/com~apple~CloudDocs\")",
      "Bash(rm -rf \"$ICLOUD/claude-commands\" \"$ICLOUD/claude-memory\")",
      "Read(//Users/[MASKED_OWNER]/Library/Mobile Documents/com~apple~CloudDocs/claude-config/**)",
      "Bash(gh pr:*)"
    ]
  },
  "hooks": {
...
`~/FAS-operations/.notebooklm-mask` 파일이 존재하는지 확인한다.
없으면 Doctrine memory에서 마스킹이 필요한 개인정보 패턴을 파악하여 생성한다.

파일 형식 (한 줄에 하나, `pattern|replacement`):
...
주의: 이 파일은 .gitignore에 포함되어야 한다.

### Step 2: Bash 스크립트 실행

    bash ~/FAS-operations/scripts/generate_notebooklm_fas.sh

이 스크립트가 자동으로 수행하는 작업:
1. 이전 결과물을 `archive/` 폴더로 이동
2. Doctrine (iCloud claude-config) 전체 파일 스캔
3. Operations (FAS-operations) 전체 파일 스캔
4. 민감정보 마스킹 (IP, 토큰, 경로, .notebooklm-mask 커스텀 패턴)
5. 코드 펜스 제거 (NotebookLM 호환)
6. 4개 파일로 분할 출력:
   - `01_doctrine.md` — Doctrine 전체
   - `02_docs_and_config.md` — Operations 문서 & 설정
   - `03_source_code.md` — Operations 소스 코드
   - `04_tests_and_scripts.md` — Operations 테스트 & 스크립트

### Step 3: 마스킹 검증

스크립트 실행 후, 아래 grep으로 누수 확인:

    grep -rn 'MASK_FILE에_등록한_실명패턴' ~/FAS-operations/reviews/notebooklm/*.md

누수가 발견되면 `.notebooklm-mask`에 패턴을 추가하고 스크립트를 재실행한다.

### Step 4: review_prompt.md 생성

`~/FAS-operations/reviews/notebooklm/review_prompt.md`를 생성한다.
이 부분은 프로젝트 맥락이 필요하므로 LLM이 직접 작성한다.

포함할 내용:
- FAS 시스템 요약 (Doctrine/Operations 구조)
...
생성된 파일 목록, 줄 수, NotebookLM 업로드 방법을 안내한다.
...
프로젝트 루트에 `.notebooklm-mask` 파일이 있으면 추가 마스킹 패턴을 적용한다.
없으면 기본 패턴(IP, 토큰, 경로)만 적용된다.
...
프로젝트에 `scripts/generate_notebooklm.sh`가 있으면 그것을 실행한다:

    bash scripts/generate_notebooklm.sh

없으면 아래 절차로 직접 스크립트를 생성하여 실행한다:

1. 프로젝트 구조를 파악한다 (src/, scripts/, docs/ 등)
2. 아래 Bash 템플릿을 프로젝트에 맞게 수정하여 실행한다:
    ...
    # 경로 마스킹
    echo "s|/Users/$(whoami)/|/Users/[MASKED_USER]/|g" >> $SED_FILE

    # IP 마스킹
    echo 's|10\.[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*|[MASKED_IP]|g' >> $SED_FILE
    echo 's|192\.168\.[0-9][0-9]*\.[0-9][0-9]*|[MASKED_IP]|g' >> $SED_FILE

    # 토큰 마스킹
    echo 's|xox[bpas]-[A-Za-z0-9_/\-]*|[MASKED_TOKEN]|g' >> $SED_FILE

    # 커스텀 마스크
    if [[ -f "$MASK_FILE" ]]; then
      while IFS='|' read -r p r; do
        [[ -z "$p" || "$p" =~ ^# ]] && continue
        echo "s|$(echo $p | xargs)|$(echo $r | xargs)|g" >> $SED_FILE
      done < "$MASK_FILE"
    fi
    ...
    if compgen -G "$OUTPUT_DIR/*.md" > /dev/null 2>&1; then
      ARCHIVE="$OUTPUT_DIR/archive/$(date +%Y-%m-%d_%H%M%S)"
      mkdir -p "$ARCHIVE"
      mv "$OUTPUT_DIR"/*.md "$ARCHIVE/"
    fi

    # 파일 수집 + 출력 (find → for loop → sed -f)
    # docs: *.md, *.yml, *.json 등 (src/ 제외)
    # source: src/**/*.ts (테스트 제외)
    # tests: src/**/*.test.ts + scripts/**

위 템플릿의 find 조건을 프로젝트에 맞게 조정한 뒤, 각 파일을 아래 형식으로 출력:
...
스크립트가 자동으로 수행하는 작업:
1. 이전 결과물 아카이브
2. 프로젝트 파일 스캔 (node_modules, .git, dist, logs 등 제외)
3. 민감정보 마스킹 (IP, 토큰, 경로, 커스텀 패턴)
4. 코드 펜스 제거 (NotebookLM 호환)
5. 3개 파일로 분할 출력:
   - `01_docs_and_config.md` — 문서 & 설정
   - `02_source_code.md` — 소스 코드 (테스트 제외)
   - `03_tests_and_scripts.md` — 테스트 & 스크립트

### Step 3: 마스킹 검증

    grep -rn '실제토큰|실명' reviews/notebooklm/*.md

누수 발견 시 `.notebooklm-mask`에 패턴 추가 후 재실행.

### Step 4: review_prompt.md 생성

`reviews/notebooklm/review_prompt.md`를 LLM이 직접 작성한다:
- 프로젝트 요약
- 검증 항목 (코드 품질, 보안, 아키텍처, 문서-코드 일치 등)
- 각 항목 [안전/주의/위험] 등급 요청
- 종합 평가 요청

### Step 5: 결과 안내

생성된 파일 목록, NotebookLM 업로드 방법 안내.

## 주의사항

- reviews/ 폴더는 .gitignore에 추가하거나, 격리 머신에는 배포하지 않는다.
- 마스킹은 **정확히 매칭되는 패턴만**. 의심스럽다고 코드를 지우지 않는다.
- 코드 로직, 변수명, 함수명, 타입 정의는 절대 마스킹하지 않는다.
...
Claude Code 스킬(슬래시 커맨드) 파일. `/명령어`로 실행 가능.
...
- 개발 환경 관련 교훈/삽질 경험도 `feedback_dev_lessons.md`에 **자동 누적 저장**할 것
...
## 헌터의 격리 상태
- macOS 유저: user (캡틴(Mac Studio #2)과 동일하지만 완전 별도 머신)
...
NotebookLM에 업로드하는 마크다운 파일에 코드 펜스(```)를 사용하면 안 됨. NotebookLM이 펜스 내부를 통째로 무시하여 "코드가 없다"고 판단함.

**Why:** 두 차례 NotebookLM 교차 검증 시도에서 동일하게 "파일명만 보이고 실제 코드가 없다"는 결과가 나옴. 5중 백틱(`````)도 마찬가지.
...
## User Profiles
- [user_overview.md](user_overview.md) — 전체 요약: 91년생 ENTJ, [MASKED_PII], 다중 직업, 월700만
...
`MEMORY.md`가 전체 메모리 파일의 인덱스. 매 세션 시작 시 자동 로드됨 (200줄 제한).
...
## 기본 정보
- 현재: [MASKED_PII] (송파구 방이동, 소규모)
- 이전: 미래탐구 목동 (의자사반 — 의대·자사고 대비반 담당)
- 출근: 주 2회 (토/일), 3시간 수업 × 3개/일 (10:00~21:00, 쉬는 시간 10분, 별도 식사시간 없음)
- 전체 학생: 약 15명
...
| 기기 | 칩 | 메모리 | 스토리지 | 현재 용도 |
|------|-----|--------|----------|----------|
| Mac Studio #1 "헌터(hunter)" | M1 Ultra | 32GB (통합) | 512GB | OpenClaw 전용 (완전 격리), macOS 유저명: user |
| Mac Studio #2 "캡틴(captain)" | M4 Ultra | 36GB (통합) | 512GB | 메인 워커 + 오케스트레이터, macOS 유저명: user |
| MacBook Pro (개인) | M1 Pro | 32GB (통합) | 512GB | 이동용, 회사 지참, macOS 유저명: [MASKED_OWNER] |
...
- Claude Max × 2 계정 (섀도우+캡틴): ~$400/월
- ChatGPT Pro (헌터/OpenClaw용): ~$200/월
- Gemini Pro × 2 계정: ~$40/월
...
## 기본 정보
- 1991년 6월 23일생 (만 34세)
- 서울 [MASKED_PII] 자취 (전세 3억, 1Gbps 인터넷)
- 여자친구 있음
- ADHD, 우울증(처방 중), 고지혈증, 고혈압, 입면장애

## 현재 역할 (다중)
1. **네이버 개발자** — 여행사업부문 > 패키지투어, 웹 풀스택 TS (월 ~500만)
2. **과학 강사** — [MASKED_PII] (토/일), EIDOS SCIENCE (월 ~200만, 300까지 성장 전망)
...
## AI 도구
- Claude Max, Gemini Pro ×2, ChatGPT Codex Pro (예정)
...
- 기기별 설정(permissions, hooks, statusLine)은 각자의 `settings.local.json`에 분리
...

{
  "statusLine": {
    "type": "command",
    "command": "bash /Users/[MASKED_USER]/.claude/statusline-command.sh"
  },
  "enabledPlugins": {
...
주인님의 **눈** — Mac Studio #1, macOS 유저명 `user`. 완전 격리 기기.
...
| 에이전트 | 기기 | 역할 |
|---------|------|------|
| 헌터(hunter) 👁️ | Mac Studio #1 (user) | 주인님의 눈 — 정보 탐색, 크롤링, 리서치 |
